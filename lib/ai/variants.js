...
export function buildVariantSystem(source) {
  return [{ type: 'text', text: variantSystemText(source), cache_control: { type: 'ephemeral' } }];
}

/** 두 경로가 나눠 쓰는 시스템 프롬프트 본문. 여기가 갈리면 같은 명령이 다른 문항을 낸다. */
function variantSystemText(source) {
...
/**
 * 프로바이더 계약(`lib/ai/provider.js`) 모양의 system 블록.
 *
 * Batch 경로의 `buildVariantSystem` 과 **본문은 같고 껍데기만 다르다** —
 * `cache_control`·`type` 은 Anthropic 전용 필드라 계약에 없다. 캐시를 걸 자리라는
 * 사실은 `cacheable` 로 남기고, 프롬프트 캐시가 없는 프로바이더는 이를 무시한다.
 * @param {string} source
 * @returns {Array<{text: string, cacheable: boolean}>}
 */
export function buildVariantSystemBlocks(source) {
  return [{ text: variantSystemText(source), cacheable: true }];
}
...
/** 변형 수를 검사해 정수로 돌려준다 (두 경로가 같은 규칙을 쓴다). */
function assertVariantCount(variantsPerItem) {
...
export function buildVariantRequests({ source, problems, variantsPerItem = DEFAULT_VARIANTS }) {
  ...
  const count = assertVariantCount(variantsPerItem);
...
/**
 * @typedef {object} VariantCall
 * @property {string} customId Batch 경로의 `custom_id` 와 **같은 값**
 * @property {string} source
 * @property {string} id 원본 문항 id
 * @property {number} variant
 * @property {Array<{text: string, cacheable: boolean}>} system
 * @property {Array<{role: 'user', content: string}>} messages
...
/**
 * 프로바이더 계약 모양의 호출 배열 — 문항 N개 × 변형 M개 = 호출 N×M 건.
 *
 * `buildVariantRequests` 의 무료 경로 짝이다. **프롬프트·스키마·customId 는 같고**
 * 껍데기만 계약 모양으로 바뀐다 (`params` 대신 `provider.completeJson` 의 인자).
 * 한쪽에서 다듬은 프롬프트가 다른 쪽에서도 그대로 도는 것이 이 대칭의 목적이다.
 *
 * @param {{source: string, problems: Array<object>, variantsPerItem?: number}} args
 * @returns {VariantCall[]}
 */
export function buildVariantCalls({ source, problems, variantsPerItem = DEFAULT_VARIANTS }) {
  const schema = variantSchema(source); // 알 수 없는 source 면 여기서 던진다
  const count = assertVariantCount(variantsPerItem);
  const system = buildVariantSystemBlocks(source);
  const schemaName = `variant_${source}`;

  const calls = [];
  for (const problem of problems) {
    for (let variant = 1; variant <= count; variant += 1) {
      calls.push({
        customId: formatCustomId({ source, id: problem.id, variant }),
        source,
        id: problem.id,
        variant,
        system,
        messages: [
          { role: 'user', content: buildVariantPrompt({ source, problem, variant, total: count }) },
        ],
        schema,
        schemaName,
        maxTokens: VARIANT_MAX_TOKENS,
        effort: VARIANT_EFFORT,
      });
    }
  }
...
// ─────────────────────────────────────────────────────────────────────────────
// 무료 경로(OpenRouter) — Batch API 가 없다
// ─────────────────────────────────────────────────────────────────────────────
//
// 무료 모델에서는 **달러 추정이 의미가 없다** (토큰 요금이 0 이다). 대신 제약은
// 호출 수다 — 분당 20회, 무입금 계정 하루 50회. `estimateVariantCost` 의 자리를
// 여기서는 `estimateFreeQuota` 가 대신한다.

/**
 * @typedef {object} FreeQuotaEstimate
 * @property {number} requestCount  보낼 호출 수
 * @property {number} dailyLimit    하루 한도
 * @property {number} perMinuteLimit 분당 한도
 * @property {number} sharePercent  하루 한도의 몇 % 를 쓰는가
 * @property {boolean} exceedsDaily 하루 한도를 넘기는가
 * @property {number} overflow      넘긴다면 몇 건이 남는가 (안 넘기면 0)
 * @property {number} minMinutes    분당 한도 때문에 최소 몇 분이 걸리는가
 */

/**
 * 이 실행이 하루 한도를 얼마나 쓰는지 미리 낸다.
 *
 * **읽을 수 없는 값은 던진다.** 한도를 어림해서 소진율을 낙관적으로 보여 주면
 * 사용자는 "괜찮겠지" 하고 돌렸다가 중간에 막힌다 (`usage.js` 가 모르는 단가를
 * 0 으로 때우지 않는 것과 같은 판단이다).
 *
 * @param {{requestCount: number, dailyLimit?: number, perMinuteLimit?: number}} args
 * @returns {FreeQuotaEstimate}
 */
export function estimateFreeQuota({
  requestCount,
  dailyLimit = OPENROUTER_FREE_LIMITS.requestsPerDay,
  perMinuteLimit = OPENROUTER_FREE_LIMITS.requestsPerMinute,
} = {}) {
  if (!Number.isInteger(requestCount) || requestCount < 0) {
    throw new Error(`requestCount 는 0 이상의 정수여야 합니다: ${requestCount}`);
  }
  if (!Number.isInteger(dailyLimit) || dailyLimit < 1) {
    throw new Error(`dailyLimit 은 1 이상의 정수여야 합니다: ${dailyLimit}`);
  }
  if (!Number.isInteger(perMinuteLimit) || perMinuteLimit < 1) {
    throw new Error(`perMinuteLimit 은 1 이상의 정수여야 합니다: ${perMinuteLimit}`);
  }
  ...
  return {
    ...
    dailyLimit,
    perMinuteLimit,
    sharePercent: (requestCount / dailyLimit) * 100,
    exceedsDaily: requestCount > dailyLimit,
    overflow: Math.max(0, requestCount - dailyLimit),
    // 첫 창은 기다리지 않으므로 창 하나를 뺀다 (20건 = 0분, 21건 = 1분).
    minMinutes: Math.max(0, Math.ceil(requestCount / perMinuteLimit) - 1),
  };
}
...
/**
 * 모델 출력에서 **비어 있으면 안 되는데 비어 있는** 필드들.
 *
 * Batch 경로와 무료 경로가 **같은 규칙**으로 판정해야 한다 — 한쪽만 느슨하면
 * 같은 명령이 프로바이더에 따라 다른 문항을 통과시킨다.
 * @param {string} source
 * @param {object} output 모델이 낸 JSON
 * @returns {string[]} 비어 있는 필수 필드 이름들 (없으면 빈 배열)
 */
export function missingVariantFields(source, output) {
  const required = REQUIRED_NONEMPTY[source];
  if (!required) throw new Error(`알 수 없는 source: ${source}`);
  return required.filter((field) => !isFilled(output?.[field]));
}
...
/**
 * 진행 기록의 항목들을 **원본 순서**로 늘어놓는다.
 *
 * 무료 경로는 여러 날에 걸쳐 이어 만들 수 있어(하루 50회 한도), 어제 만든 것과 오늘
 * 만든 것이 섞인다. 파일에 도착 순서로 쌓으면 이어 돌릴 때마다 diff 가 뒤집힌다 —
 * `collectVariantResults` 가 Batch 결과에 쓰는 규칙(원본 순서 → 변형 번호)과 같게 맞춘다.
 *
 * 원본에 없는 항목은 버리지 않고 뒤로 민다 — 조용히 사라지면 왜 개수가 줄었는지
 * 알 수 없다. 계약 위반은 `validateGeneratedDoc` 가 잡는다.
 *
 * @param {Array<{customId: string, item: object}>} entries
 * @param {Array<object>} originals
 * @returns {Array<object>} 항목 배열
 */
export function sortVariantEntries(entries, originals = []) {
  const order = new Map(originals.map((item, index) => [item.id, index]));

  return [...entries]
    .map((entry, index) => {
      const parsed = parseCustomId(entry.customId);
      return {
        item: entry.item,
        rank: [
          order.get(parsed?.id ?? entry.item?.variantOf) ?? Number.MAX_SAFE_INTEGER,
          parsed?.variant ?? Number.MAX_SAFE_INTEGER,
          index, // 같은 자리면 들어온 순서 — 정렬이 흔들리지 않게
        ],
      };
    })
...
export async function collectVariantResults({ results, source, originals }) {
  ...
  for await (const row of results) {
    ...
    const missing = missingVariantFields(source, output);
    if (missing.length > 0) {
