...
- ~~문항 단위 딥링크는 못 걸었다~~ → **2026-09-04 해소.** `useDeepLink.js` 훅으로 세 페이지가
  `?id=`를 읽는다. 커서를 `useState` 숫자로 두면 목록이 fetch 뒤 도착하는 화면에서 딥링크를
  반영할 수단이 effect뿐이라, **상태에는 "사용자가 옮긴 위치"만 담고 실제 위치는 렌더 중 파생**한다.
  못 찾는 id(지운 오답·토글 꺼진 변형·손으로 고친 URL)는 첫 문항 폴백 + 원인별 안내.
...
- ~~모의고사 결과는 `quiz_results`에 쓰지 않는다~~ → **2026-09-04 해소.** `exam_results` 별도 키 +
  스냅샷 `examResults` 필드로 약점 분석까지 흘러간다. 같은 문항이 겹칠 때의 우선순위는
  **오답노트 추정 < `quizResults` 확정 < `examResults` 확정** — 두 맵에 타임스탬프가 없어
  "더 최근"을 고를 수 없으므로 순서를 규칙으로 못 박았고, 모의고사가 실전 조건(정답 가림·시간 제한)이라
  실력 추정으로 낫다. 단 모의고사의 `'answered'`(정오 미상)는 코드 퀴즈의 확정 판정을 밀어내지 못한다.
- **`partial`은 `'incorrect'`로 접어 저장한다.** 저장 계약에 중간값이 없다.
- ~~클라이언트가 `userAnswer`를 미리 자르지 않는다~~ → **2026-09-04 해소.** 보내기 전에 자르고
  `truncated`·`originalLength`·`sentLength`를 호출부에 알린다(뒷부분이 채점에서 빠진 걸 모르면
  점수를 오해한다). 상한은 의존성 없는 `lib/ai/limits.js` 하나를 서버·브라우저가 함께 import 한다.
...
- ~~`validateTutorBody`·`validateGradeBody`가 제어문자를 지우지 않는다~~ → **2026-09-04 해소.**
  `	`·`
`·`
`은 보존하고 나머지 C0/C1만 제거한다(기존 `CONTROL_CHARS` 상수 재사용).
  `
`을 남기는 이유: 윈도우 textarea가 `
`을 보내는데 `
`만 지우면 **사용자가 적지 않은 답안을
  서버가 만들어낸다.** 순서도 정리 → 빈 값 검사 → 길이 검사로 바꿨다 — `trim()`은 NUL·ESC를 공백으로
  보지 않아 제어문자만 적힌 답안이 채점으로 넘어가고 있었다.
- 레이트리밋 LRU 축출이 삽입 순서 기준이라 가장 활발한 IP가 먼저 버려질 수 있다(정확한 한도는 공유 저장소 몫).

### 백로그 정리 (2026-09-04)

Phase 0~5 를 닫은 뒤 그동안 "남긴 것"으로 적어 둔 항목을 서브에이전트 3개로 정리했다.
**테스트 1350 → 1469**, lint 0, build 성공, `npm audit` 0건.

- **의존성 취약점 10건 전건 해소**(high 8·moderate 1·low 1 — "high 2건"이라는 기존 기록이 틀렸다).
  메이저 승격 없이 끝났다. react-router 권고 9건 중 SSR/RSC 전용은 정적 SPA라 도달 불가지만
  **`<Link>`·`useNavigate`의 백슬래시·`//` open redirect 2건은 클라이언트 라우팅만으로 도달**한다.
  **락파일만 고치면 안 된다** — `package.json`의 선언 범위 하한도 올려야 락파일 없는 `npm install`이
  다시 취약 버전을 집지 않는다. 락파일·선언 범위 양쪽을 검사하는 회귀 테스트를 뒀다.
- `npm audit`을 CI 스텝으로 넣지 않았다 — 레지스트리에 네트워크로 묻는 명령이라 우리 코드와
  무관하게 CI를 빨갛게 만든다(작업 중 실제로 503을 받았다). 대신 위 회귀 테스트로 대체한다.
- **FlashcardPage 의 로드 콜백에 있던 `setIdx(0)`이 카드가 도착할 때마다 커서를 0으로 밀고 있었다**
  (딥링크 작업 중 발견). 리셋을 덱 버튼 핸들러로 옮겼다.
...
**닫힌 라이브 검증 항목**
- **`includeFiles: public/data/**` 가 실제로 먹는다** — 유효한 id(`002`)는 키 검사까지 가고(502),
  없는 id(`999`)는 `quiz100 에 id 999 인 문항이 없습니다`로 400. **둘이 갈렸다는 건 람다가 교재 md 를
  읽고 파싱했다는 뜻**이다(파일이 없으면 모든 id 가 400).
- **SPA rewrite 가 `/api/*` 를 삼키지 않는다** — 함수에 도달.
- **레이트리밋이 프로덕션에서 동작** — 10회 후 429 + `Retry-After: 5` + `retryAfterSeconds`.
- **body 검증** — source 화이트리스트·빈 답안 모두 400 + 사람이 읽는 메시지.
- **P3 수정이 프로덕션 화면에서 확인됨** — `/quiz?id=S-01` 에서 SQL 쿼리와 예제 테이블이 함께 보인다.
- **문항 딥링크** — `?id=C-07` 진입, 없는 id 는 첫 문항 + 안내, 플래시카드 덱 자동 전환.
- **AI 실패의 우아한 저하** — 해설/채점 패널이 사람이 읽는 안내와 "다시 시도"를 띄우고,
  **문항·코드·풀이·추적표·함정과 자기 채점 버튼(맞았어요/틀렸어요)이 그대로 남는다.**
  채점 실패 문구가 "다시 시도하거나 **직접 채점해 주세요**"로 폴백 경로를 가리킨다.
- **생성물 없음이 정상 상태** — `/data/generated/*.json` 404 를 오류로 취급하지 않는다.
- 모바일(390px) 가로 스크롤 0px.

**남은 것은 전부 키가 필요하다** — 스트리밍 실제 흐름, `cache_read_input_tokens > 0`,
플래너 60초 완주, 구조화 출력+도구 조합의 400 여부, 평가셋 30건 일치율, 비용 실측치.

**AI 를 켜는 순서 (중요)**: 공개 URL 이므로 **접근 코드를 먼저** 걸고 그다음 API 키를 넣는다.
